...
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QTableWidget, QTableWidgetItem, \
    QPushButton, QDialog, QFormLayout, QLineEdit, QCheckBox, QComboBox, QLabel, QMessageBox, QPlainTextEdit
...
class ClientWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.table_compras.setColumnCount(6)  # Atualizar o número de colunas
        self.table_compras.setHorizontalHeaderLabels(
            ["Número do Serviço", "ID", "Data do Agendamento", "Serviço", "Valor da Venda", "Notas"])
    ...
    def load_compras(self):
        try:
            ...
            for row_idx, compra in enumerate(compras):
                for col_idx, value in enumerate(compra[:5]):
                    item = QTableWidgetItem(str(value))
                    self.table_compras.setItem(row_idx, col_idx, item)

                    # Carregar o campo de "Notas" (último campo)
                    notas_item = QTableWidgetItem(compra[5])  # Acessar o índice correto para as notas
                    self.table_compras.setItem(row_idx, 5, notas_item)  # Colocar as notas na última coluna
                    ...
                    if compra_data:
                        ...
                        if dialog.exec_():
                            ...
                            editar_compra(
                                ...
                                dados_atualizados.get('notas', None),
                            )
    ...
    def initUI(self):
        ...
        if self.compra_data:
            try:
                ...
                self.input_notas.setPlainText(self.compra_data[5])
        ...
        layout.addRow("Data do Agendamento:", self.input_data_venda)
        layout.addRow("Serviço:", self.input_produto)
